 ...
 * Hooks Test Suite
 *
 * NOTE: Some tests are skipped because oh-my-droid has different module
 * implementations than oh-my-claudecode. The keyword-detector module in
 * oh-my-droid has a simpler API without extractPromptText, removeCodeBlocks,
 * detectKeywordsWithType, hasKeyword, getPrimaryKeyword exports.
 *
 * Tests for Todo Continuation, Hook Output Structure, UltraQA Loop,
 * and Mutual Exclusion are active and working.
 */
import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { mkdirSync, writeFileSync, rmSync } from 'fs';
import { join } from 'path';
import { tmpdir } from 'os';
// NOTE: oh-my-droid keyword-detector has different API - these exports don't exist:
// extractPromptText, removeCodeBlocks, detectKeywordsWithType, hasKeyword, getPrimaryKeyword, DetectedKeyword
import {
  formatTodoStatus,
  getNextPendingTodo,
  type Todo,
...
// NOTE: resetTodoContinuationAttempts doesn't exist in oh-my-droid persistent-mode
import {
  startUltraQA,
  clearUltraQAState,
  isRalphLoopActive
...
// Mock DetectedKeyword type for type-only usage in tests that still compile
...
// SKIPPED: oh-my-droid keyword-detector has different API
...
// SKIPPED: oh-my-droid keyword-detector has different API
describe.skip('Integration: Keyword Detection with Code Blocks', () => {
...
describe.skip('Edge Cases', () => {
  ...
  describe('Unicode and special characters', () => {
    it('should handle unicode characters', () => {
      const text = 'search for files with emojis';
      expect(hasKeyword(text)).toBe(true);
    });
...
// SKIPPED: oh-my-droid persistent-mode doesn't export resetTodoContinuationAttempts
describe.skip('Persistent Mode - Max Attempts Counter', () => {
  const testSessionId = 'test-session-123';

  it('should export resetTodoContinuationAttempts function', () => {
    // Skipped - function doesn't exist in oh-my-droid
...
describe('Mutual Exclusion - UltraQA and Ralph', () => {
  ...
  describe('isUltraQAActive', () => {
    ...
    it('should return true when ultraqa is active', () => {
      const stateFile = join(testDir, '.omd', 'ultraqa-state.json');
  ...
  describe('isRalphLoopActive', () => {
    ...
    it('should return true when ralph is active', () => {
      const stateFile = join(testDir, '.omd', 'ralph-state.json');
      writeFileSync(stateFile, JSON.stringify({ active: true }));
  ...
  describe('UltraQA mutual exclusion', () => {
    it('should fail to start UltraQA when Ralph is active', () => {
      ...
      const ralphStateFile = join(testDir, '.omd', 'ralph-state.json');
  ...
  describe('Ralph mutual exclusion', () => {
    it('should fail to start Ralph when UltraQA is active', () => {
      ...
      const ultraqaStateFile = join(testDir, '.omd', 'ultraqa-state.json');
