 ...
 * 2. Active ralph loop (until cancelled via /cancel)
...
4. When FULLY complete (after Architect verification), run \`/omd-cancel\` (or \`/cancel\`) to cleanly exit and clean up state files. If cancel fails, retry with \`/omd-cancel --force\`.
